...
interface ProjectCardProps {
  project: Project;
  imageUrl?: string; // Optional direct override
  index: number;
...
const ProjectCard: React.FC<ProjectCardProps> = ({ project, imageUrl, index, onClick }) => {
  ...
  // Use the override URL if provided, otherwise use coverImageUrl, fallback to hero imageUrl
  const displayImageUrl = imageUrl || project.coverImageUrl || project.imageUrl;
    ...
    >
      ...
      <div className="relative z-20 p-8 md:p-16 pt-16 md:pt-28 flex flex-col items-center w-full h-full pointer-events-none">
        <div className="space-y-4 [backface-visibility:hidden] [transform:translateZ(0)]">
          <h3 className="text-3xl md:text-5xl font-semibold tracking-tight text-[#1a1a1a] drop-shadow-sm">
            {project.title}
          </h3>
          <p className="text-lg md:text-xl text-[#3c4043] font-medium max-w-md mx-auto leading-relaxed drop-shadow-sm">
            {project.description}
          </p>
        </div>
      </div>
